"""
kaggle_baseline.py — Self-contained baseline evaluation for Kaggle Notebooks
=============================================================================

Kaggle gives 2× T4 GPUs (30 GB VRAM total) — models load in float16, no
4-bit quantization required. Larger batch sizes are possible.

The CSV is accessed from a Kaggle Dataset you attach to the notebook.
Results are saved to /kaggle/working/results/.

Run this script from a Kaggle notebook cell:
    !python /kaggle/working/demographic-aware-steering/kaggle_baseline.py \\
        --csv-path /kaggle/input/<your-dataset>/Demo-SafetyBench.csv \\
        --models qwen \\
        --batch-size 16
"""
...
# Default CSV path on Kaggle — update <dataset-slug> to match your dataset.
# Format: /kaggle/input/<dataset-owner>/<dataset-slug>/Demo-SafetyBench.csv
DEFAULT_CSV_PATH = "/kaggle/input/demo-safetybench/Demo-SafetyBench.csv"
...
def load_model_and_tokenizer(model_id: str):
    """Load a causal-LM model in float16 across all available GPUs.

    Kaggle 2×T4 provides 30 GB VRAM total — the 7B model fits in float16
    without 4-bit quantization. device_map="auto" spreads layers across
    both GPUs automatically.
    """
    ...
    print(f"  Loading model:     {model_id} (float16, multi-GPU)")
    model = AutoModelForCausalLM.from_pretrained(
        model_id,
        torch_dtype=torch.float16,
        device_map="auto",          # splits across both T4 GPUs automatically
...
def generate_responses(
    ...
    batch_size: int = 16, max_new_tokens: int = 50,
) -> List[str]:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run baseline evaluation on Demo-SafetyBench for Table 1 (Kaggle)."
    )
    parser.add_argument("--csv-path", type=str, default=DEFAULT_CSV_PATH,
                        help="Path to Demo-SafetyBench.csv (Kaggle input directory).")
    parser.add_argument("--models", type=str, nargs="+",
                        choices=list(MODEL_REGISTRY.keys()),
                        default=list(MODEL_REGISTRY.keys()))
    parser.add_argument("--output-dir", type=str, default="/kaggle/working/results")
